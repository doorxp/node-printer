...
# Build Electron Windows 64bit
node-gyp configure --target=$version --arch=x64 --dist-url=https://electronjs.org/headers --module_name=node_printer
node-gyp build package --runtime=electron --target=$version --target_arch=x64 --build-from-source

# Build Electron Windows 32bit
node-gyp configure --target=$version --arch=ia32 --dist-url=https://electronjs.org/headers --module_name=node_printer
node-gyp build package --runtime=electron --target=$version --target_arch=ia32 --build-from-source